...
acl_size = 5
-- aclsrc_tbl = Dynamic("aclsrc0", (acl_size, ["srcmac"]))
-- aclsrc_val = Static([[("srcmac", 0)] | x <- [1..acl_size]])

-- acldst_tbl = Dynamic("acldst0", (acl_size, ["dstmac"]))
-- acldst_val = Static([[("dstmac", 0)] | x <- [1..acl_size]])
...
-}
    --, MKT(aclsrc_tbl, aclsrc_val)
    --, MKT(acldst_tbl, acldst_val)
    , MKT(acl_tbl, acl_val)
